...
#PowerVC - vm3 configuration
#----------------------------------
variable "vm3_storage_name" {
  description = "Storage Pool where to place the new VM"
}

variable "vm3_image_name" {
  description = "Image Name for PowerVC instance"
}

variable "vm3_first_ip" {
  description = "vm3 First IP to be used"
}
...
locals {
  ...
  #------------------------------------------------------------------------------------------------#
  #vm3_flavor_name = "${var.vm3_name}-flavor-${uuid()}"
  vm3_flavor_name = "${var.vm3_name}-flavor"

  #VCPU RATIO 1:5 1 vcpu = 0.2 pcpu (cores)
  vm3_vcpu_ratio = "${var.vm3_vcpu_ratio}"
  ...
  val_vm3_max_memory = "${var.vm3_memory < local.vm3_max_memory ? local.vm3_max_memory : var.vm3_memory * 2}"
  ...
  val_vm3_max_cpu = "${var.vm3_cpu < local.vm3_max_cpu ? local.vm3_max_cpu : var.vm3_cpu * 2}"

  #IP - cluster:
  vm3_ip_octets = split(".", "${var.vm3_first_ip}")
...
################### DATASOURCES ####################
####################################################
data "openstack_images_image_v2" "vm3-image-name" {
  name        = "${var.vm3_image_name}"
  most_recent = true
}
...
###################################################################################
resource "openstack_compute_servergroup_v2" "vm3-servergroup-affinity" {
  count    = "${var.vm3_number > 1 ? 1 : 0}"
  name     = "${var.vm3_name}-servergroup"
...
###################################################################################
resource "openstack_compute_flavor_v2" "vm3-flavor" {
  count = "${var.vm3_number > 0 ? 1 : 0}"
  name  = "${local.vm3_flavor_name}"
  ram   = "${var.vm3_memory * 1024}"
  vcpus = "${var.vm3_cpu}"
  disk  = "0"
  ...
  extra_specs = {
    ...
    "powervm:srr_capability"          = "${var.vm3_remote_restart}"
    "powervm:min_mem"                 = "${local.vm3_min_memory * 1024}"
    "powervm:max_mem"                 = "${local.val_vm3_max_memory * 1024}"
    "powervm:min_vcpu"                = "${local.vm3_min_cpu}"
    "powervm:max_vcpu"                = "${local.val_vm3_max_cpu}"
    "powervm:min_proc_units"          = "${local.vm3_min_cpu / local.vm3_vcpu_ratio}"
    "powervm:proc_units"              = "${var.vm3_cpu / local.vm3_vcpu_ratio}"
    "powervm:max_proc_units"          = "${local.vm3_max_cpu / local.vm3_vcpu_ratio}"
  }
}
...
resource "openstack_compute_instance_v2" "vm3" {
  ...
  count = "${var.vm3_number}"

  depends_on = [
    "openstack_compute_flavor_v2.vm3-flavor",
    "openstack_compute_instance_v2.vm1",
    "openstack_compute_instance_v2.vm2",
  ]

  name         = "${format("${var.vm3_name}-%02d", count.index + 1)}"
  image_id     = "${data.openstack_images_image_v2.vm3-image-name.id}"
  power_state  = "${var.vm3_power_state}"
  flavor_id    = "${openstack_compute_flavor_v2.vm3-flavor[0].id}"
  force_delete = "true"

  availability_zone = element("${data.openstack_compute_availability_zones_v2.AZ.names}", count.index + 1)
  #Network configuration
  network {
    name = "${var.net1_name}"
    #uuid = "${openstack_networking_network_v2.net1.id}"
    fixed_ip_v4 = "${local.vm3_ip_octets[0]}.${local.vm3_ip_octets[1]}.${local.vm3_ip_octets[2]}.${local.vm3_ip_octets[3] + count.index}"
  }
...
#####################################################################################
resource "null_resource" "vm3_post_install_config" {

  depends_on = ["openstack_compute_instance_v2.vm3"]
  ...
  connection {
    ...
    host     = "${openstack_compute_instance_v2.vm3[count.index].network.0.fixed_ip_v4}"
    timeout  = "15m"
  }
...
resource "openstack_blockstorage_volume_v3" "vm3_dockerdisk1" {

  depends_on = ["null_resource.vm3_post_install_config"]

  count = "${var.vm3_dockerdisk1 > 0 ? "${var.vm3_number}" : 0}"
  #count                = "${var.vm3_number}"
  description          = "${format("${var.vm3_name}-%02d Docker Disk1", count.index + 1)}"
  name                 = "${format("${var.vm3_name}-%02d-docker-disk-1", count.index + 1)}"
  size                 = "${var.vm3_dockerdisk1}"
  enable_online_resize = "true"
  volume_type          = "${var.vm3_storage_name}"
}

#Attach Volume dockerdisk1
resource "openstack_compute_volume_attach_v2" "vm3_va_dockerdisk1" {

  depends_on = ["openstack_blockstorage_volume_v3.vm3_dockerdisk1"]

  count = "${var.vm3_dockerdisk1 > 0 ? "${var.vm3_number}" : 0}"
  #count       = "${var.vm3_number}"
  volume_id   = "${openstack_blockstorage_volume_v3.vm3_dockerdisk1[count.index].id}"
  instance_id = "${openstack_compute_instance_v2.vm3[count.index].id}"
